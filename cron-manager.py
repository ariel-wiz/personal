...
scripts_to_run = [
    {
        ScriptExecution.path: "/Users/ariel/PycharmProjects/webhook/notion.py",
...
def main():
    ...
    for script in scripts_to_run:
        if script.get(ScriptExecution.frequency) and (today == script.get(ScriptExecution.frequency) or script.get(ScriptExecution.frequency) == 'daily'):
            ...
            subprocess.run(['python3', script[ScriptExecution.path], script[ScriptExecution.arg]], check=True)
        else:
